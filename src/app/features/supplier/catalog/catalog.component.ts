          ...
          <table class="w-full text-sm">
            ...
            <tbody>
              <tr *ngFor="let sc of catalog" class="border-t border-gray-800/40 hover:bg-white/[0.02] transition">
                <td class="px-5 py-3 text-white font-medium">{{ sc.componentName }}</td>
                <td class="px-5 py-3">
                ...
                <td class="px-5 py-3">
                  ...
                  <div *ngIf="editingId === sc.id" class="flex gap-2">
                    ...
                    <button (click)="editingId = null" class="text-xs px-2.5 py-1 rounded-lg bg-gray-500/15 text-gray-400 hover:bg-gray-500/25 transition">Cancel</button>
                  </div>
                </td>
              </tr>
...
export class SupCatalogComponent implements OnInit {
    ...
    constructor(private supCompService: SupplierComponentService, private compService: ComponentService) { }

    ngOnInit() {
        this.load();
        this.compService.getAll().subscribe({ next: d => this.masterComponents = d || [], error: () => { } });
    }

    load() { this.supCompService.getMyCatalog().subscribe({ next: d => this.catalog = d || [], error: () => { } }); }

    addToCatalog() {
        this.supCompService.addToCatalog(this.newItem).subscribe({
            next: () => { this.showAddForm = false; this.newItem = { componentId: null, unitPrice: 0, stockQuantity: 0, leadTimeDays: 7 }; this.load(); },
        ...
        this.supCompService.updatePrice(id, this.editPrice).subscribe({
            next: () => {
                this.supCompService.updateStock(id, this.editStock).subscribe({
                    ...
                    error: () => { this.editingId = null; this.load(); }
                });
            },
            error: (e: any) => alert(e.error?.message || 'Error')
        });
