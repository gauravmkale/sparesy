...
@Component({
    ...
    <div>
      ...
      <div *ngIf="selectedProject" class="mb-6">
        <button (click)="selectedProject = null" class="text-gray-500 hover:text-white text-sm mb-4 flex items-center gap-1 transition">← Back to list</button>
...
export class ClientProjectsComponent implements OnInit {
    ...
    constructor(private projectService: ProjectService, private prodService: ProductionService) { }

    ngOnInit() { this.load(); }

    load() { this.projectService.getMyProjects().subscribe({ next: d => this.projects = d || [], error: () => { } }); }

    selectProject(p: any) {
        this.selectedProject = p;
        this.prodService.getByProject(p.id).subscribe({ next: d => this.productionOrder = d, error: () => this.productionOrder = null });
    }

    submitProject() {
        this.projectService.submitProject(this.newProject).subscribe({
            next: () => { this.showForm = false; this.newProject = { name: '', quantity: 100, layerCount: 2, boardThickness: 1.6, surfaceFinish: 'HASL' }; this.load(); },
